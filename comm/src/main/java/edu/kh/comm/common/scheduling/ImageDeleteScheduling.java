...
public class ImageDeleteScheduling {
	...
	@Autowired
	private ServletContext application; // application scope 객체 -> 서버 폴더 경로 얻어오기 위해 사용
	
	// 스케줄링에 사용되는 메서드는 무조건 'public void 메서드명()' 이어야 함
	...
	public void serverImageDelete() {
		...
		if( !serverList.isEmpty() ) { // 서버에 이미지 파일이 있을 때 비교/삭제 진행
			...
			for(File serverImage : serverList) {
				...
				if(dbList.indexOf(name) == -1) { // List에 value 와 같은 값이 있으면 인덱스 반환, 없으면 -1 반환
					
					// == dbList에는 없는데, serverList에는 존재하는 경우
